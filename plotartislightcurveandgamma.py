...
def main():
    glcfiles = glob.glob('gamma_light_curve.out') + glob.glob('*/gamma_light_curve.out') + glob.glob('*/*/gamma_light_curve.out')
...
def makeplot(lcfiles,glcfiles):
    ...
    for n in range(len(lcfiles)):
        ...
        ax.plot(arraytime, lightcurvedata[:,1]+gammalightcurvedata[:,1], lw=1.5, linestyle=linestyle, label="Sum")
