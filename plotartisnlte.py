...
def main():
    ...
    fig, axes = plt.subplots(nions, 1, sharex=False, figsize=(8,10), tight_layout={"pad":0.2,"w_pad":0.0,"h_pad":0.0})

    for ion, ax in enumerate(axes):
        ax.plot(list_levels[ion], list_ltepop[ion], lw=1.5, label='LTE', linestyle='None', marker='x')
        ax.plot(list_levels[ion], list_nltepop[ion], lw=1.5, label='NLTE', linestyle='None', marker='x')
